...
import matrizen.core.DateiManager.Musik;
...
public class Spiel implements KeyListener {
	...
	private Spiel() {
		...
		MusikPlayer.setVolume(.00000001f);
	 ...
	 * = new Text(0, "Mit der Leertaste, E, Q oder STRG kannst du",
	 ...
	 * if (gegnerEntfernen()) level.hinzufuegen(new Item(Item.Typ.schluessel,
	 * new Vektor(7, 4))); } if (ticks == 1000) { text = new
	...
	private void tutorialAbarbeiten() {
		if (tutorial) {
			if (ticks == 0) {
				level = Level.level0;
				text = new Text(1, "Bewege dich mit den WASD-Tasten oder Pfeil-", "tasten druch die Welt");
				SpielFenster.gibInstanz().addKeyListener(this);
			...
			else if (ticks > tutorialTick + tutorialDelay && ticks < tutorialTick + tutorialDelay * 2.0
					&& tutorials[6])
				text = new Text(0, "Alles andere, was du über das Spiel wissen",
						"musst, erfährst du im weiteren Verlauf");
			else if (ticks > tutorialTick + tutorialDelay * 2.0 && tutorials[6]) {
				text = new Text(-1, "Alles andere, was du über das Spiel wissen",
						"musst, erfährst du im weiteren Verlauf");
